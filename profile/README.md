...
[**stapik-cloud**](https://github.com/Stapik-Group/stapik-cloud) is the self-hosted sync backend the apps above talk to — document storage with versioning and conflict resolution, per-app API keys, and a JWT-secured admin panel to manage it all. It's the odd one out stack-wise (Java/Spring Boot + Next.js instead of C++/GTK4), since it runs on your server, not your desktop. Ships with a one-shot deployment script for Proxmox VE. Entirely optional — every app above works fully offline without it.

## Design choices worth knowing about

- **Local-first.** Every app writes to `~/.local/share/<appname>/` and works fully offline. Cloud sync is opt-in, talks to a self-hosted API you control (see [stapik-cloud](https://github.com/Stapik-Group/stapik-cloud) above), and resolves conflicts by keeping whichever copy was saved more recently — the whole document at once, no field-by-field merging. Simple, predictable, and good enough for one person using a couple of machines.
...
Windows and Flatpak support are on the roadmap for the whole suite. Check each repo's own TODO for what's coming to that particular app.
